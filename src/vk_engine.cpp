...
void VulkanEngine::init_vulkan()
{
    ...
    auto inst_ret = builder.set_app_name("Example Vulcan Application")
    ...
    // we want a gpu that can write to sdl surface and support vulkan with 1.3 the correct features
    ...
    // Get the vkdevice handle usde in the rest of a vulkan application
    _device = vkbDevice.device;
    _chosenGPU = physical_device;
...
void VulkanEngine::create_swapchain(uint32_t width, uint32_t height)
{
    ...
    // for the draw image, we want to allocate it from gpu to local memory
...
void VulkanEngine::init_commands()
{
    ...
    // we also want the pool to allow for resetting of individual command buffer
...
{
    // wait until the gpu finished rendering the last frame.Timeout of 1 second
    ...
    // begin the command buffer recording, we will use this command buffer exactly once, wo se want to let vulkan know that
    VkCommandBufferBeginInfo cmdBeginInfo = vkinit::command_buffer_begin_info(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
    ...
    // we will siganl the _renderSemaphore, to signal that rendering has finished
    ...
    // submit command buffer to the queue and execute it.
    //  _renderFences will now block until the graphic commands finish execution
    VK_CHECK(vkQueueSubmit2(_graphicsQueue, 1, &submit, get_current_frame()._renderFence));

    // prepare present
    //  this will put the image we just rendered to into the visible window
    //  we want to wait on the _renderSemaphore fot that.
...
void VulkanEngine::init_background_pipelines()
{
    ...
    // store the created pipeline handle in the effect so binding and destruction use a valid pipeline
    gradient.pipeline = _gradientPipeline;

    // cchange the shader module onlu to create the sky shader
    ...
    // add 2 background effect into the array
    backgroundEffects.push_back(gradient);
    backgroundEffects.push_back(sky);

    // destroy structure properly
...
void VulkanEngine::init_imgui()
{
    ...
    _mainDeletionQueue.push_function([&]()
		...
		vkDestroyDescriptorPool(_device, imguiPool, nullptr); }); // imguiPool is captured by value
}
